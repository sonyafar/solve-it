...
@Component({
  selector: 'app-secant-method',
  templateUrl: './secant-method.component.html',
  styleUrls: ['./secant-method.component.css']
})
export class SecantMethodComponent implements OnInit {
  ...
  constructor(private viewportScroller: ViewportScroller, private chartService: ChartService) {}
  ...
  getResult(): number {
    ...
    if((x0 != undefined) && (x1 != undefined)) {
      ...
      do
      {
        if(this.function(x1) == this.function(x0)) {
          throw new Error('DivideByZeroException');
        }

        x2 = -(this.function(x0) * (x1 - x0)) / (this.function(x1) - this.function(x0)) + x0;
